// <copyright file="updateResource.tsx" company="Microsoft Corporation">
...
import { Text, Flex, Input, Loader, Button, TextArea } from "@fluentui/react-northstar";
import { TFunction } from "i18next";
import './group.scss';
import Constants from "../../constants/constants";
import { ResourceEntity } from "../../models/resource";
import { updateResource, getResource } from "../../apis/resourceApi";
...
class UpdateResource extends React.Component<IUpdateGroupProps, IState> {
    localize: TFunction;
    ...
    public async componentDidMount() {
        ...
        if ('id' in params) {
            let id = params['id'];
            await this.getResourceDetails(id);
            this.setState({
                loading: false,
                resourceId: id
            });
    ...
    */
    private handleSubmit = () => {
        if (this.checkIfSubmitAllowed()) {
            ...
            // Update resource details
            this.updateResourceDetails(resourceDetails);
        }
    }

    /**
   *Update group from API
   */
    private async updateResourceDetails(resourceDetails: any) {
        try {
            let response = await updateResource(resourceDetails);
            if (response && response.status === 200 && response.data) {
...
export default withTranslation()(UpdateResource)